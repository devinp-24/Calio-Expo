...
const API_BASE: string = extra?.API_BASE ?? "http://206.87.217.55:3001/api";
...
`.trim();
  ...
  async function handleAssistantButton(value?: string) {
    ...
    // Vendor buttons → the special flow you already built
    if (value === "uber-eats" || value === "doordash") {
      return orderWithApp(value as "uber-eats" | "doordash");
    }
